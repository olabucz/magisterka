...
/**/		vector <int> w_publikacje;
			...
			for (int k = 0; k < 100; k++)
			{
				...
				{
					w.push_back(i + 1);
/**/				w_publikacje.push_back(i + 1); //albo niżej przypisać wektor w do w_publikacje
				} 
...
/**/							//w.push_back(w[rand() % (w.size() - j)]); 	// rand() % w.size() -j - dostęp do losowego indeksu, przy rosnącej dlugosci wektora, ale (- j) zapewnia stałą obszar losowania dla danego segmentu			
								...
								{
/**/								w.push_back(w_publikacje[rand() % p + (w_publikacje.size() - p)]); //chyba ok
